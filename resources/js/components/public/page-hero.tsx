...
export default function PageHero({ eyebrow, title, description, children }: PropsWithChildren<PageHeroProps>) {
    return (
        <section className="relative overflow-hidden border-b border-white/10 bg-ink">
            <div className="absolute inset-0 bg-gradient-to-br from-navy/40 via-ink to-ink" />
            <div className="relative mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8 lg:py-24">
                {eyebrow && (
                    <p className="flex items-center gap-3 text-xs font-medium tracking-[0.3em] text-marine/90 uppercase">
                        <span className="h-px w-10 bg-marine/60" />
                        {eyebrow}
                    </p>
                )}
                <h1 className="mt-5 text-4xl font-light tracking-tight text-white sm:text-5xl lg:text-6xl">{title}</h1>
